...
function getRemForAggr(uri, qry) {
	...
	.each(function() {rem=this.rem.value.toString()});
...
function extract_canvas_size(qry, uri) {
	...
	qry.where('<' + uri + '> exif:height ?height')
		...
		.each(function() {
			h = this.height.value; w = this.width.value;
...
function cb_process_manifest(qry, uri) {
	...
	.each(function() {seqs.push(this.seq.value);});
	qry.reset();
	...
	.each(function() {
		...
		what = this.seq.value;				
		if (c == '*') { imgAnnos[c].push(what) 
		} else if (imgAnnos[c] == undefined) { imgAnnos[c] = [what]
		} else { imgAnnos[c].push(what) }	
	})
	...
	.each(function() { 
		c = (this.canv == undefined) ? '*' : this.canv.value;	
		what = this.seq.value;
		if (c == '*') { textAnnos[c].push(what) 
		} else if (textAnnos[c] == undefined) { textAnnos[c] = [what]
		} else { textAnnos[c].push(what) }	
	})
	.end()
	...
	.each(function() { 
		...
		c = (this.canv == undefined) ? '*' : this.canv.value;	
		what = this.seq.value;
		if (c == '*') { audioAnnos[c].push(what) 
		} else if (audioAnnos[c] == undefined) { audioAnnos[c] = [what]
		} else { audioAnnos[c].push(what) }	
	})
	.end()
	...
	.each(function() { 
		c = (this.canv == undefined) ? '*' : this.canv.value;	
		what = this.seq.value;
		if (c == '*') { zoneAnnos[c].push(what) 
		} else if (zoneAnnos[c] == undefined) { zoneAnnos[c] = [what]
		} else { zoneAnnos[c].push(what) }	
	})
	.end()
	...
	.each(function() { 
		c = (this.canv == undefined) ? '*' : this.canv.value;	
		what = this.seq.value;
		if (c == '*') { commentAnnos[c].push(what) 
		} else if (commentAnnos[c] == undefined) { commentAnnos[c] = [what]
		} else { commentAnnos[c].push(what) }	
	})
	qry.reset();
	...
	.each(function() {ranges.push([this.title.value, this.f.value])});
...
function cb_process_sequence(qry, uri) {
	...
	.each(function() {top = this.seq.value;});
...
function cb_process_annoList(qry, uri) {
	...
	if (uri != undefined) {
		...
		.each(function() {top = this.seq.value.toString()});
		if (top != null) {
	...
	} catch (e) { alert('error building annos: ' + e) }
