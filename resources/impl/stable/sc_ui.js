...
function populate_jumpMenu(qry) {
	...
	if (ranges.length) {
		...
		idxl.sort(function(a,b){return a-b});
...
function showPages() {
	...
	topinfo['raphaels'] = {'image':{},'text':{},'audio':{},'zone':{},'comment':{}};
...
function showCanvas(canvas, canvasId) {	
	...
	if (1) {
		...
		cud.position({'of':'#' + canvasId, 'my':'left top', 'at':'left top','collision':'none', 'offset':'20 -10'});
		var cudi = $('#canvasUriInput_'+canvasId);
		cudi.click(function() {$(this).select();});
...
function mk_raphael(typ, canvas, canvasId) {
	...
	if (topinfo['raphaels'][typ][canvas] != undefined) {
		...
		svg.position({'of':'#'+canvasId, 'my':'left top', 'at':'left top', 'collision':'none'});	
...
function show_zpr(what) {
	...
	$('#zpr-close').click(function() { $('#zpr-wrap').remove();$('#zpr-fullscreen').remove(); $('#zpr-close').remove();});
    ...
    fn = basefile + fn;
    // alert(fn);
    var z = new zpr('zpr-fullscreen', { 'jp2URL': fn, 'width': w , 'height': h, 'marqueeImgSize': 150  });
...
function paint_imageAnno(anno, canvasId) {
	...
	if (anno.body.options.length > 0) {
		...
		anno.body.options.sort(function(a,b){return a.id > b.id ? 1 : -1});
		if (img == null) {
		...
		$('.imgSelRadio').click(function() {alert($(this).attr('id'))})
	...
	if (img.rotation == 0 && anno.targets[0].partOf == null && img.partOf == null) {
		...
		var div = $('#' + myid)
		div.position({'of':'#'+canvasId, 'my':'left top', 'at':'left top', 'collision':'none', 'offset': '0 0'});
		...
		zprs.position({'of':'#' + canvasId, 'my':'left top', 'at':'left top','collision':'none', 'offset':'2 2'});
		var zprb = $('#zprb_'+canvasId);
		zprb.click(function() {show_zpr(imguri)});
		...
		if (clip) {
			...
			img.offset({top:div.position().top - imgofftop, left:div.position().left-imgoffleft});
			img.css('clip', clip);
		}
		img.height(imgh);
		img.width(imgw);

		div.position({'of':'#'+canvasId, 'my':'left top', 'at':'left top', 'collision':'none', 'offset': offset});
...
function paint_textAnno(anno, canvasId) {
	...
	if (xywh != null) {
		...
		danno.position({'of':'#' + canvasId, 'my':'left top', 'at':'left top','collision':'none', 'offset': offset});
...
function paint_audioAnno(anno, canvasId) {
	...
	if (xywh != null) {
		...
		aanno.position({'of':'#' +canvasId, 'my':'left top', 'at':'left top', 
			'collision':'none', 'offset': offset});
	}

    $('#audio_button_' + myid).click(audio_button_click);
    $('#audio_button_' + myid).attr('canvasId', canvasId);
    $("#audio_progress_" + myid).progressbar({value: 0}).css({height:10, opacity: 0.0});
	if (!$('#check_show_audio').is(':checked')) {aanno.hide();}
}
...
function audio_button_click(e) {
	...
	if (img == '/imgs/play.png') {	
		...
		if (curr != null) {
			...
			pb.progressbar({value:0}).css({'opacity':0.0});			
		}		
...
function on_audio_currentTime(e) {
	...
	var durn = end - start;
	var relv = ct - start;							
	l.progressbar({value: relv / durn * 100});
	if (ct > end) {
		$(this).jPlayer('stop');
		$('.audio_button').attr('src', '/imgs/play.png')
		l.progressbar({value:0});
		l.css({opacity:0.0});
	}
}
...
function reorder_layers(e) {
	...
	ids = [];
	$('#show_body').children().each(function() {ids.push($(this).attr('id'))});
	...
	for (var x=0;x<nCanvas;x++) {
		if (x != 0) {
			if (x % perrow == 0) {
				...
				$('#canvas_'+x).position({'of':'#canvas_' + (x-perrow), 'my':'left top', 'at':'left bottom', 
						'collision':'none', 'offset': '0 10'});
			} else {
				$('#canvas_' +x).position({'of':'#canvas_' + (x-1), 'my':'left top', 'at':'right top', 
					'collision':'none', 'offset': '10 0'});
			}
		}
	}	
}
